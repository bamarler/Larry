...
class UISettingsPage(SettingsPage):
    ...
    def create_font_settings(self):
        font_label = tk.Label(self.page, text="Font Settings", font=(FONT, FONTSIZE, 'bold'), bg=HEADER_COLOR, fg=TEXT_COLOR)
        font_label.pack(pady=10)

        font_frame = tk.Frame(self.page, bg=BACKGROUND_COLOR)
        font_frame.pack(fill=tk.X, padx=10, pady=5)

        font_family_label = tk.Label(font_frame, text="Font Family:", font=(FONT, FONTSIZE), bg=BACKGROUND_COLOR, fg=TEXT_COLOR)
        ...
        font_size_slider = tk.Scale(font_frame, from_=8, to=16, orient=tk.HORIZONTAL, variable=self.font_size_var, bg=BACKGROUND_COLOR, fg=TEXT_COLOR)
        font_size_slider.pack(side=tk.LEFT, padx=5)

    def create_theme_settings(self):
        theme_label = tk.Label(self.page, text="Theme Settings", font=(FONT, FONTSIZE, 'bold'), bg=HEADER_COLOR, fg=TEXT_COLOR)
        ...
        for theme_name, colors in themes.items():
            theme_radio = tk.Radiobutton(theme_frame, text=theme_name, variable=self.theme_var, value=theme_name, font=(FONT, FONTSIZE), bg=BACKGROUND_COLOR, fg=TEXT_COLOR, indicatoron=0)
            theme_radio.pack(fill=tk.X, padx=5, pady=2, anchor='w')

            color_frame = tk.Frame(theme_frame, bg=BACKGROUND_COLOR)
            color_frame.pack(fill=tk.X, padx=20, pady=2, anchor='w')
    ...
    def create_save_button(self):
        save_button = tk.Button(self.page, text="Save", command=self.save_settings, font=(FONT, FONTSIZE), bg=ACCENT_COLOR, fg=TEXT_COLOR)
